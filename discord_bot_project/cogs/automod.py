...
class AutoMod(commands.Cog):
    def __init__(self, bot):
        ...
        self.time_window = 5  # 초 단위로 스팸 체크 시간 간격
    ...
    @commands.command(name="경고초기화", help="사용자의 경고를 초기화합니다 (관리자 전용).")
    @commands.has_permissions(administrator=True)
    async def reset_warnings(self, ctx, member: discord.Member):
        # 경고 초기화
        if member.id in self.warnings:
            del self.warnings[member.id]
            await ctx.respond(f"{member.mention}의 경고 기록이 초기화되었습니다.")
        else:
            await ctx.respond(f"{member.mention}에게 부여된 경고 기록이 없습니다.")

    @commands.command(name="경고확인", help="사용자의 현재 경고 횟수를 확인합니다.")
    async def check_warnings(self, ctx, member: discord.Member):
        # 경고 횟수 확인
        warning_count = self.warnings.get(member.id, 0)
        await ctx.respond(f"{member.mention}의 현재 경고 횟수는 {warning_count}회입니다.")
